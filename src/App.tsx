...
function loadActivities(): Activity[] { try { return JSON.parse(localStorage.getItem(LS_KEY) || "[]"); } catch { return []; } }
function saveActivities(items: Activity[]) { localStorage.setItem(LS_KEY, JSON.stringify(items)); }
function saveApiKey(k: string) { localStorage.setItem(LS_GMAPS, k || ""); }
...
function renderTimeRange(a: Activity) {
  ...
  const start = parseTimeToMinutes(a.time)!; const end = start + a.durationMinutes; const eh = Math.floor((end % (24*60))/60); const em = end % 60;
  return `${a.time}–${pad(eh)}:${pad(em)}`;
}
function computeOverlaps(items: Activity[]) {
  const set = new Set<string>(); const byDate = groupBy(items, i => i.date);
  for (const [, arr] of byDate) {
    const rows = arr.map(i => { const s=parseTimeToMinutes(i.time); const e = s!=null && i.durationMinutes ? s+i.durationMinutes : s ?? -1; return { id:i.id, s:s??-1, e }; }).sort((x,y)=>x.s-y.s);
    for (let i=0;i<rows.length;i++) for (let j=i+1;j<rows.length;j++) {
      const A=rows[i],B=rows[j]; if (A.s<0||B.s<0) continue; const aEnd=Math.max(A.e,A.s), bEnd=Math.max(B.e,B.s);
      if (!(aEnd <= B.s || bEnd <= A.s)) { set.add(A.id); set.add(B.id); }
    }
    const same = groupBy(rows.filter(r=>r.s>=0 && r.s===r.e), r=>r.s); for (const [,list] of same) if (list.length>1) list.forEach(r=>set.add(r.id));
  }
  return set;
}
...
function parseCoordsFromUrl(url: string) {
  try {
    if (!url) return null;
    const re = /!3d(-?\d+(?:\.\d+)?)!4d(-?\d+(?:\.\d+)?)/g; let m: RegExpExecArray | null; let last: {lat:number;lng:number}|null=null;
    while ((m = re.exec(url)) !== null) { const lat=parseFloat(m[1]), lng=parseFloat(m[2]); if (!Number.isNaN(lat) && !Number.isNaN(lng)) last={lat,lng}; }
    if (last) return last;
    try { const u = new URL(url, "https://example.invalid"); const q = u.searchParams.get("q") || u.searchParams.get("query") || u.searchParams.get("ll");
      if (q) { const [latS,lngS]=q.split(","); const lat=parseFloat(latS), lng=parseFloat(lngS); if (!Number.isNaN(lat)&&!Number.isNaN(lng)) return {lat,lng}; }
    } catch {}
    const at = url.indexOf("@"); if (at!==-1) { const seg=url.slice(at+1).split(/[\/?#]/)[0]; const [latS,lngS]=seg.split(","); const lat=parseFloat(latS), lng=parseFloat(lngS);
...
function loadGoogleMaps(key: string): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    const s = document.createElement("script"); s.src = `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(key)}`; s.async = true;
    s.onload = () => resolve((window as any).google.maps); s.onerror = () => reject(new Error("Failed to load Google Maps JS"));
...
/* =====
   UI
   ===== */
function Button({ children, onClick, variant, className }: { children: React.ReactNode; onClick?: ()=>void; variant?: "black"|"grey"|"ghost"; className?: string }) {
      ...
      <div className="grid grid-cols-7 gap-2 text-sm">
        ...
        {cells.map((d, i) => (
          <div key={i} className="min-h-[90px] border border-neutral-200 rounded-xl p-2 bg-white">
            ...
            {d !== null && byDate.get(`${y}-${pad2(m + 1)}-${pad2(d)}`)?.slice(0, 4).map(a => (
...
function MapView({ apiKey, items }: { apiKey: string; items: Activity[] }) {
  ...
  const activityPins = React.useMemo(()=> items
    .map(a=> ({ a, coord: a.location? parseCoordsFromUrl(a.location) : null }))
    .filter(x=> !!x.coord) as { a:Activity, coord:{lat:number;lng:number} }[], [items]);

  const savedPins = React.useMemo(()=> savedPlaces.map((ft:any)=>({
  ...
  return (
    <div>
      ...
      <div className="text-sm text-neutral-500 mt-3">Upload a GeoJSON to show saved places. Activities come from their Location field.</div>
    </div>
  );
}
      ...
      <main className="max-w-6xl mx-auto px-4 pt-20 pb-24">
        ...
        {tab==="map" && (
          <Card>
            ...
            <div className="text-neutral-600 mb-4">Activities are red. Saved places are black. Duplicates are merged by coordinates; the activity wins.</div>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-50 bg-black/30 grid place-items-center p-4">
          <div className="bg-white rounded-2xl border border-neutral-200 w-full max-w-2xl p-5">
            ...
            <Input label="Google Maps API key" value={apiKey} onChange={(e:any)=>setApiKey(e.target.value)} placeholder="paste your key" />
            <div className="font-semibold mt-6 mb-2">Sync with Firebase</div>
            <TextArea label="Firebase config (paste the JSON object only)" rows={6} value={fbConfigText} onChange={(e:any)=>setFbConfigText(e.target.value)} placeholder='{"apiKey":"...","authDomain":"...","projectId":"...","appId":"..."}' />
            <Input label="Share code" value={fbShareCode} onChange={(e:any)=>setFbShareCode(e.target.value)} placeholder="e.g., lil-trip" />
            <label className="flex items-center gap-2 mb-3 text-sm"><input type="checkbox" checked={syncEnabled} onChange={(e:any)=>setSyncEnabled(e.target.checked)} /> Enable auto-sync</label>
            <div className="text-xs text-neutral-500 mb-4">Use the same config and share code on each device. Anonymous sign-in only. Last write wins.</div>
            <div className="flex justify-end"><Button onClick={()=>setShowSettings(false)}>Close</Button></div>
          </div>
        </div>
      )}
    </div>
